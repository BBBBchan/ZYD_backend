...
@picture_manage.route('/add_category', methods=['POST'])
@checkLogin
def add_category():
    data = request.json
...
@picture_manage.route('/change_category', methods=['POST'])
@checkLogin
def change_category():
    data = request.json
...
@picture_manage.route('/delete_category/<int:category_id>')
@checkAdmin
...
# 增加作品标签，删除仅可管理员后台操作
@picture_manage.route('/add_tag',methods=['POST'])
@checkAdmin
def add_tag():
    data = request.json
...
# 管理员是否同意图片上推荐
@picture_manage.route('/judgment_apply', methods=['POST'])
@checkAdmin
def judgment_apply():
    data = request.json
